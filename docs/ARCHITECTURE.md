...
базовый прогноз и чистый расчёт потребности через CLI. Работают PostgreSQL,
сохраняемый импорт, миграции и неизменяемые наборы. API, worker,
...
Текущая реализация CLI: `import-file` / `import-all` синхронно сохраняют входы,
`dataset-create` явно фиксирует выбор источников. PostgreSQL advisory lock защищает
повторный импорт; savepoint откатывает строки при сбое. Триггеры запрещают изменение
завершённых импортов и активированных манифестов. Очередь/worker описаны ниже как следующий слой.
Физическая схема первого этапа описана в [POSTGRES_IMPORT.md](POSTGRES_IMPORT.md).
