...
Расширено расчётное ядро: сезонность, устойчивый тренд, кандидаты в разовые продажи,
восстановление спроса при подтверждённом отсутствии товара и запас по ошибке прогноза.
Девять воспроизводимых сценариев запускаются командой `python -m app.cli engine-demo` из backend.
Методика и ограничения: [docs/DEMAND_ENGINE.md](docs/DEMAND_ENGINE.md).
...
Подключён расчёт ИЭК из Excel и PostgreSQL с датами поставок и явными правилами
метр/бухта: [запуск и ограничения](docs/IEK_PREVIEW.md).
Следующий этап: API, сохраняемые задания, интерфейс и полный сценарий утверждения.
