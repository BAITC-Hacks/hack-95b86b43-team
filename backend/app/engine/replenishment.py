...
def calculate(inputs: ReplenishmentInput) -> ReplenishmentResult:
    ...
    # Stock is the opening position of calculation_date. Receipts arrive before that day's demand.
    ...
    steps = [{"operation": "horizon_days", "lead_days": inputs.lead_days,
              "review_days": inputs.review_days, "value": horizon},
             {"operation": "forecast_demand", "value": demand},
             {"operation": "eligible_inbound", "value": inbound_total}]
    if inputs.available_stock is None:
        return ReplenishmentResult("insufficient_data", demand, inbound_total, None, None,
                                   None, "unknown", tuple(steps), tuple(limitations + ["Available stock is unknown"]))
    ...
    steps.append({"operation": "max_zero_demand_plus_safety_minus_stock_minus_inbound",
                  "demand": demand, "safety_stock": inputs.safety_stock,
                  "available_stock": inputs.available_stock, "inbound": inbound_total, "value": raw})
    if raw == 0:
        quantity = ZERO
    elif any(v is None for v in (inputs.storage_units_per_order_unit, inputs.minimum_order, inputs.order_multiple)):
        return ReplenishmentResult("insufficient_data", demand, inbound_total, raw, None, deficit,
                                   urgency, tuple(steps), tuple(limitations + ["Conversion, minimum order or multiple is unknown"]))
    else:
        in_order_units = raw / inputs.storage_units_per_order_unit
        quantity = (max(in_order_units, inputs.minimum_order) / inputs.order_multiple).to_integral_value(
            rounding=ROUND_CEILING) * inputs.order_multiple
        steps.append({"operation": "convert_apply_minimum_round_up", "requirement_in_order_units": in_order_units,
                      "conversion": inputs.storage_units_per_order_unit, "minimum": inputs.minimum_order,
                      "multiple": inputs.order_multiple, "value": quantity})
    steps.append({"operation": "order_quantity", "value": quantity})
    return ReplenishmentResult("calculated", demand, inbound_total, raw, quantity, deficit,
                               urgency, tuple(steps), tuple(limitations))
