...
Частичный сентябрь и будущие месяцы исключены. Это сохранённый режим `baseline-1`;
расширенная модель доступна с отдельной политикой, описанной ниже.
...
Расчёт ИЭК подключён; дальше — API и worker с сохранением расчётов,
согласование вопросов [аудита](../docs/DATA_AUDIT.md) и пользовательский процесс.
...
Есть режим `--dataset` для PostgreSQL. В примере начальный остаток сентября
используется как явно условный запас, актуальным снимком он не считается.
Правила метр/бухта задаются отдельно по коду товара.
Подробности — [IEK_PREVIEW.md](../docs/IEK_PREVIEW.md).

## Расширенное ядро и демонстрация

Без PostgreSQL и исходных Excel:

```powershell
.venv/Scripts/python -m app.cli engine-demo --seed 42
Start-Process ../data/cache/engine-demo.html
.venv/Scripts/python -m unittest discover -s ../tests/acceptance -v
```

Генерируются девять синтетических сценариев, JSON, HTML и диагностический CSV.
Для нового прогноза на реальных исходниках:
...
Можно добавить `--dataset` с ID сохранённого набора для чтения из PostgreSQL.
Описание алгоритмов, проверки и ограничения — [DEMAND_ENGINE.md](../docs/DEMAND_ENGINE.md).
